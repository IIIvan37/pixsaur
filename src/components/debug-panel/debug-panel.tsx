...
export const DebugPanel: React.FC<DebugPanelProps> = ({ className }) => {
  ...
  }, [isEnabled, timingEnabled, refreshStats])
  ...
  return (
    ...
    >
      ...
      {isEnabled && (
        <div style={{ marginBottom: '8px' }}>
          <div style={{ display: 'block', marginBottom: '4px' }}>Niveau:</div>
          <select
            value={level}
            onChange={(e) =>
              handleLevelChange(e.target.value as LoggerConfig['level'])
            }
            style={{
              background: '#333',
              color: 'white',
              ...
              timers.length > 0 && (
                ...
                >
                  ...
                  {timers.map((timer) => (
                    <div
                      key={timer}
                      style={{ paddingLeft: '8px', color: '#0f0' }}
                    >
                      • {timer}
                    </div>
                  ))}
