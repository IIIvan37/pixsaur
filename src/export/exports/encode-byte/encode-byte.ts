...
/**
 * Encode pixels into a CPC byte for the given mode.
 *
 * @param indexBuf - The index buffer containing palette indices
 * @param x - The starting x position in the buffer
 * @param y - The y position (line number)
 * @param mode - CPC video mode (0, 1, or 2)
 * @param width - Buffer width
 * @param stride - Distance between pixels in buffer (default 1, use 2 for EGX doubled pixels)
 */
export function encodeByte(
  ...
  switch (mode) {
    case 0: {
      ...
      const right = indexBuf[px + stride] & 0x0f
    ...
    case 1: {
      const c0 = indexBuf[px + 0 * stride] & 0x03
      const c1 = indexBuf[px + 1 * stride] & 0x03
      const c2 = indexBuf[px + 2 * stride] & 0x03
      const c3 = indexBuf[px + 3 * stride] & 0x03
    ...
    case 2: {
      for (let i = 0; i < 8; i++) {
        const c = indexBuf[px + i * stride] & 0x01
        byte |= c << (7 - i)
      }
      break
    }
