...
// Helper function to check if colors are equal
function colorsEqual(color1: Vector, color2: Vector): boolean {
  return (
    color1[0] === color2[0] &&
    color1[1] === color2[1] &&
    color1[2] === color2[2]
...
// Helper function to validate preselected colors inclusion
function validatePreselectedInclusion(
  result: readonly Vector[],
  preselected: readonly Vector[]
): void {
  preselected.forEach((preselectedColor) => {
    const found = result.some((resultColor) =>
      colorsEqual(resultColor, preselectedColor)
    )
    expect(found).toBe(true)
  })
}

// Helper function to validate a color vector
function validateColorVector(color: Vector): void {
...
// Helper function to validate result array
function validateResultArray(
  result: readonly Vector[],
  maxColors: number
): void {
  ...
  expect(result.length).toBeLessThanOrEqual(maxColors)

  result.forEach(validateColorVector)
}
...
describe('ReGLQuantizer', () => {
  ...
  describe('quantizePalette', () => {
    ...
    it('should respect preselected colors', async () => {
      ...
      validatePreselectedInclusion(result, preselected)
    })
  ...
  describe('GPU Methods', () => {
    ...
    it('should call ReGL read for color selection', async () => {
      const histogram = [10, 5, 3, 8, 0, 2, 1, 4] // Array comme attendu par selectColorsGPU
