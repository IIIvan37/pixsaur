...
describe.skipIf(!BENCHMARK_CONFIG.ENABLE_REAL_BENCHMARKS)(
  'ReGL Quantizer Real Benchmarks',
  () => {
    ...
    it('should provide baseline CPU performance metrics', async () => {
      ...
      for (const size of BENCHMARK_CONFIG.IMAGE_SIZES) {
        for (const targetColors of BENCHMARK_CONFIG.TARGET_COLORS) {
          ...
          // Log pour analyse
          console.log(
            `${size.name} ${targetColors} colors: ${avgTime.toFixed(2)}ms avg (${minTime.toFixed(2)}-${maxTime.toFixed(2)}ms)`
          )
        }
      }
      ...
      if (smallResults.length > 0 && largeResults.length > 0) {
        const avgSmall =
          smallResults.reduce((sum, r) => sum + r.avgTime, 0) /
          smallResults.length
        const avgLarge =
          largeResults.reduce((sum, r) => sum + r.avgTime, 0) /
          largeResults.length
...
describe('ReGL Quantizer Theoretical Performance Tests', () => {
  ...
  it('should calculate theoretical GPU advantages', () => {
    ...
    scenarios.forEach((scenario) => {
      ...
      console.log(
        `${scenario.description}: Theoretical GPU speedup ${theoreticalSpeedup.toFixed(1)}x`
      )
  ...
  it('should identify GPU bottlenecks and optimization opportunities', () => {
    ...
    console.log(
      'High-impact GPU optimizations available:',
      highImpactOptimizations.map((b) => b.operation)
    )
  })
})
